...
const getDashboardStats = asyncHandler(async (req, res) => {
  ...
  if (isAdminRequest) {
    ...
    // Apply global mask to count, expenses & totals
    ...
    // Apply global quantity mask on topItems for count and replace name if rule applies
    mappedTopItems = topItems.map(item => ({
      _id: getReplacedName(item._id, rules),
      count: maskQuantityValue(item.count, rules),
      revenue: maskCurrencyValue(item.revenue, null, rules)
    }));

    // Apply global mask to hourly sales totals
