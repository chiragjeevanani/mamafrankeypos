  ...
  return (
    <nav className={`h-14 border-b shrink-0 flex items-center px-5 justify-between transition-colors duration-500 z-[100] ${
      ...
      <div className="flex items-center gap-3">
        <div className="w-9 h-9 bg-[#E1261C] rounded-xl flex items-center justify-center text-[#D4AF37] shadow-md shadow-[#E1261C]/20">
          <ChefHat size={18} />
        </div>
        <div className="hidden sm:flex flex-col">
          <span className={`font-black text-sm uppercase tracking-tighter ${isDarkMode ? 'text-white' : 'text-[#E1261C]'}`}>Kitchen Display</span>
          ...
          return (
            <NavLink
              ...
              className={`flex items-center gap-2 px-3 py-2 rounded-lg text-[11px] font-black uppercase tracking-widest transition-all ${
                isActive 
                  ? 'bg-[#E1261C] text-white shadow-md' 
                  : (isDarkMode ? 'text-stone-300 hover:text-white hover:bg-white/8' : 'text-stone-500 hover:text-[#E1261C] hover:bg-white')
              }`}
            >
              <item.icon size={15} />
