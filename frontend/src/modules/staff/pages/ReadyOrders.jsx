  ...
  return (
    <div className="min-h-screen bg-staff-bg flex flex-col font-sans">
      <header className="sticky top-0 z-40 bg-staff-card/95 backdrop-blur-xl border-b border-staff-border px-6 py-8">
        <h1 className="text-2xl font-black text-staff-text-primary tracking-tight mb-1">Pickups</h1>
        <p className="text-[10px] font-black uppercase tracking-widest text-staff-text-muted">Ready to be served</p>
      </header>
                ...
                >
                  <div className="flex items-start justify-between mb-6">
                    <div className="flex items-center gap-4">
                      <div className="w-14 h-14 bg-staff-success rounded-2xl flex items-center justify-center text-staff-text-white font-black text-xl shadow-lg shadow-emerald-500/20">
                        {order.table}
                      </div>
                      <div>
                        <div className="flex items-center gap-1.5 mb-0.5">
                          <Hash size={10} className="text-emerald-600" />
                          <p className="text-xs font-black text-staff-text-primary leading-none">{order.orderNum}</p>
                        </div>
                        <p className="text-[9px] font-black text-emerald-600 uppercase tracking-widest">Ready for pickup</p>
                      </div>
                    </div>
                    <div className="bg-staff-card/80 backdrop-blur-sm px-3 py-1.5 rounded-full border border-emerald-200 flex items-center gap-2">
                  ...
                  <div className="space-y-3 mb-6">
                    {order.items.map((item, idx) => (
                      <div key={idx} className="flex items-center gap-3">
                        <div className="w-1.5 h-1.5 rounded-full bg-staff-success" />
                        <span className="text-sm font-bold text-staff-text-primary">{item.quantity}x {item.name}</span>
                      </div>
                    ))}
                  </div>
                  ...
                  <motion.button
                    ...
                    className="w-full py-4 bg-staff-success text-staff-text-white rounded-2xl font-black uppercase tracking-widest text-[11px] shadow-lg shadow-emerald-500/30 flex items-center justify-center gap-2"
                  >
